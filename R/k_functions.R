...
# this function is under development, it should reduce the time
# for the caluclation of k functions
# randomize_distmatrix2 <- function(graph, edge_df, n, start_vert = NULL, resolution = NULL){
...
#
#
#   new_pts <- lapply(1:nrow(edit_edge), function(i){
#     edge <- edit_edge[i,]
#     ## b : create fictive nodes along that line
#     dists <- seq(resolution, edge$weight,by = resolution)
#     all_nodes <- c(start_node ,end_node)
#      <- edge$start_oid
#      <- edge$end_oid
#
#   })
#
#   ## c : add the fictive nodes to the network
#   ## d : create the connexions between the added nodes
#
#   ## step2 : select the nodes that will be starting events
#
# }
